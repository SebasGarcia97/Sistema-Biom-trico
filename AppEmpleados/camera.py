...
class VideoCamera(object):
	def __init__(self):               
		...
		#self.video = cv2.VideoCapture('rtsp://169.254.205.112:8080/h264_pcm.sdp')
		self.count=0
	def __del__(self):
		self.video.release()
	def get_frame(self):                
	...
	def CapturarRostros(self):
		...
		if self.count <= 50:
			...
			self.count = self.count + 1
			if ban == True:
				cv2.putText(frame,"Espere mientras se Entrena", (0,80), cv2.FONT_HERSHEY_SIMPLEX, 1 ,(0,255,0), 2, cv2.LINE_AA)
			else:
